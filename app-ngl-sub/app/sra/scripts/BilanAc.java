 ...
 *
 */
public class BilanAc extends Script<BilanAc.MyParam>{
	...
	public void execute(MyParam args) throws Exception {	
		for (String submissionCode : args.codes) {
			...
			if (submission.release) {
				printfln(" %s est une soumission pour release du study %s", submissionCode, submission.studyCode);
			} else {
				printfln("Liste des AC attribues pour la soumission %s", submissionCode);
				if (submission.studyCode != null && StringUtils.isNotBlank(submission.studyCode)) {
					...
					if (StringUtils.isNotBlank(study.accession)) {
						printfln("   - studyCode = %s, AC = %s,  bioproject= %s", study.code, study.accession, study.externalId);
					} else {
						printfln("   - studyCode = %s", study.code);
					}
				}
				if (submission.sampleCodes != null) {
					for (int i = 0; i < submission.sampleCodes.size() ; i++) {
						...
						if (StringUtils.isNotBlank(sample.accession)) {
							printfln("   - sampleCode = %s, AC = %s,  externalId= %s", sample.code, sample.accession, sample.externalId);
						} else {
							printfln("   - sampleCode = %s", sample.code);
						}
					}					
				}
				if (submission.experimentCodes != null) {
					for (int i = 0; i < submission.experimentCodes.size() ; i++) {
						Experiment experiment = experimentAPI.dao_getObject(submission.experimentCodes.get(i));
						if (StringUtils.isNotBlank(experiment.accession)) {
							printfln("   - experimentCode = %s, AC = %s", experiment.code, experiment.accession);
						} else {
					...
					for (int i = 0; i < submission.experimentCodes.size() ; i++) {
						...
						if (StringUtils.isNotBlank(experiment.run.accession)) {
							printfln("   - runCode = %s, AC = %s", experiment.run.code, experiment.run.accession);
						} else {
							printfln("   - runCode = %s", experiment.run.code);
						}
					}					
				}
				printfln("  ");
