...
public class ScriptTools {
	...
	public  Map<String, String> getListRawData (String submissionCode) throws IOException, SraException {
		...
		if (submission == null) {
			logger.error("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA Dans base, soumission inexistante pour ", submissionCode);
			return mapRawData;
		} else {
			logger.error("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX  Recuperation dans la base de la soumission" , submissionCode);
		}
		...
		if (submission.experimentCodes.isEmpty()) {
			logger.error("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA Aucun experimentCodes dans " , submission.code );
			return mapRawData;
		} else {
			logger.error("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXX La soumission contient des experimentCodes");
		}
		...
		for (String experimentCode : submission.experimentCodes) {
			...
			for(RawData rawData : run.listRawData) {
				...
				if(!relatifName.endsWith("gz")) {
					...
					//logger.error("ajout extention gz => new relatifName" + relatifName);
				}
				...
				if (directory.startsWith("/ccc/")) {
					...
					logger.error("directory ccc transformé en cns_directory " + cns_directory);
				}
