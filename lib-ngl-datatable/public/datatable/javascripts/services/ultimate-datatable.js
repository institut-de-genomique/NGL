        ...
        var datatable = {
            configDefault: {
                ...
                exportCSV: {
                    ...
                    delimiter: ";",
                    start: false,
                    extraHeaders: false // should export extraHeaders in csv
                },
                otherButtons: {
                    active: false,
                ...
                if (this.config.search.active && this.isRemoteMode(this.config.search.mode)) {
                    ...
                    if (url) {
                        ...
                        $http.get(url(), {
                            ...
                            if (resp != null && resp != undefined && typeof resp.data !== 'string') {
                                ...
                                that.config.messages.clazz = that.config.messages.errorClass;
                                that.config.messages.text = "Erreur technique - Si l'erreur persiste, veuillez contacter le support NGL.";
                            }
                        }).catch(function (error) {
                            that.setSpinner(false);

                            that.config.messages.clazz = that.config.messages.errorClass;
                            that.config.messages.text = "Erreur technique (" + error.status + ") - La requête semble trop volumineuse. Si l'erreur persiste, veuillez contacter le support NGL.";
                        });
                    ...
                    currentDisplayResult.forEach(function(result, i) {
                        currentColumns.forEach(function(column, j) {
                            if (i > 0 && column.mergeCells) {
                                if (getColumnValue(column, result) === getColumnValue(column, previousResult)) {
                                    rowspans[i][j] = 0;
                                    nbRowEquals[j]++;
                                    // last element
			 ...
			 */
             loadUrlColumnProperty: function() {
                ...
                urlColumns.forEach(function(column) {
                    if (column.url.indexOf("&codes") >= 0) {
                        var re = new RegExp('\'', 'g');
                        var requestRes = column.url.substring(0, column.url.indexOf("&codes")).replace(re, ''); 

                        displayResult.forEach(function(value) {
                            var url = $parse(column.url)(value);

                            var substrUrl = url.substring(url.indexOf('&codes'));

                            if (requestRes.length > 20000) {
                                arrayReq.push(requestRes);
                                requestRes = column.url.substring(0, column.url.indexOf("&codes")).replace(re, '');
                            } 
                            requestRes += substrUrl;
                        });

                        arrayReq.push(requestRes);

                        arrayReq.forEach(function (req) {
                            $http.get(req, {
                                url: req
                            }).then(function(result) {    
                                result.data.forEach(function (res) {
                                    var urlRes = result.config.url.substring(0, result.config.url.indexOf("&codes"));
                                    urlRes += "&codes=" + res.code;
                        ...
                        displayResult.forEach(function(value) {
                            ...
                            if (!angular.isDefined(urlCache[url])) {
                                ...
                                urlQueries.push($http.get(url, {
                                    ...
                                    for(var j = 0; j < urlCache[result.config.url].length; j++) {
                                        urlCache[result.config.url][j].objectBeforeUrl = value;
                                    }
                                },function(result){
            ...
            setMessagesActive: function(isActive) {
                this.config.messages.active = isActive;
            },
            ...
            remove: function() {
                if (this.config.remove.active && !this.config.remove.start) {
                	if ($window.confirm(this.messages.Messages("datatable.remove.confirm"))) {
                        this.setSpinner(true);
                        this.config.messages.text = undefined;
                        this.config.messages.clazz = undefined;
                ...
                if (this.config.select.active && this.displayResult) {
                    ...
                    for (var i = 0; i < this.displayResult.length; i++) {
                        ...
                        if (angular.isFunction(this.config.select.callback)) {
							console.warn('select.callback is deprecated. Use mouseevents.clickCallback instead.');
            ...
            getFinalValue : function(column, result){
            	...
            	if (!result.line.group && (column.url === undefined || column.url === null)) {
                    ...
                    if (typeof this.urlCache[url] == 'string') { // Si c'est un string, c'est qu'il y a eu une erreur et donc un message d'erreur.
                                                                 // Sinon on a un objet en JSON.
                        v = this.urlCache[url];
                    } else {
                        v = $parse(column.property+filter)(this.urlCache[url]);
                    }
            ...
            exportCSV: function(exportType) {
                if (this.config.exportCSV.active) {
                    ...
                    if (displayResultTmp) {
                        ...
                        //extra-headers
                        if(this.config.exportCSV.extraHeaders) {
                            var getExtraHeaders = function(columns) {
                                return columns
                                .filter(isShowingColumn)
                                .map(getExtraHeader);
    
                                //---
    
                                function isShowingColumn(column) {
                                    return !that.config.hide.columns[column.id];
                                }
    
                                function getExtraHeader(column) {
                                    return isExtraHeaderPresent() ? getExtraHeaderValue() : null;
    
                                    //---
        
                                    function isExtraHeaderPresent() {
                                        return column.extraHeaders && column.extraHeaders[0];
                                    }
        
                                    function getExtraHeaderValue() {
                                        var extraHeader = column.extraHeaders[0];
                                        return angular.isFunction(extraHeader) ? extraHeader() : that.config.messages.transformKey(extraHeader);
                                    }
                                }
                            };
    
                            var notAllNull = function(extraHeaders) {
                                return extraHeaders.some(isNotNull);
    
                                //---
    
                                function isNotNull(extraHeader) {
                                    return extraHeader !== null;
                                }
                            };
    
                            var extraHeadersLine = function(extraHeaders) {
                                return extraHeaders
                                .map(replaceNullValues)
                                .join(delimiter) + "\n";
    
                                //---
    
                                function replaceNullValues(extraHeader) {
                                    return extraHeader === null ? "" : extraHeader;
                                }
                            };
                            
                            var extraHeaders = getExtraHeaders(columnsToPrint);
                            if(notAllNull(extraHeaders)) lineValue = extraHeadersLine(extraHeaders);
                        }
                        // header
					  ...
					  // if ngModel is not defined, we don't need to do
	      		     ...
	      		     scope.getItems = function(){
	      		    	 if(scope.isEdit() && scope.filter){
	      		    		var filter = {};
	      		    		$parse(optionsConfig.viewMapper.replace(optionsConfig.itemName+'.','')).assign(filter, filterValue);
	...
	return {
                ...
                link: function(scope, element, attrs, ngModelController) {
                	...
                	ngModelController.$formatters.push(function(value) {
        			    return convertValues.convertValue(value, property.saveMeasureValue, property.displayMeasureValue);
        			}); 
  		    	...
  		    	link: function(scope, element, attr) {
  		    		...
  		    		var udtTableHelpers = {
  		    				...
  		    				setImage : function(imageData, imageName, imageFullSizeWidth, imageFullSizeHeight) {
  		  		    			...
  		  		    			var zoom = Math.min((document.body.clientWidth - 25) / imageFullSizeWidth, 1);
