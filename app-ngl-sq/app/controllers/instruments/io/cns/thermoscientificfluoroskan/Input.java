...
public class Input extends AbstractInput {
	...
	public Experiment importFile(Experiment experiment, PropertyFileValue pfv, ContextValidation contextValidation) throws Exception {		
		...
		if(experiment.instrument.inContainerSupportCategoryCode.equals("tube")){
			...
			//Nom de la plaque en A20 premier onglet
			plateCodeInFile = getStringValue(sheet.getRow(19).getCell(0));
		...
		if (plateCodeInExp.equals(plateCodeInFile)) {
			...
			if (experiment.typeCode.equals("reception-fluo-quantification") || 
				...
				//Get type QC en A2 premier onglet
				//Valeurs possibles : Dosage HS1, Dosage HS2, Dosage HS3 et Dosage BR
				String typeQC = getStringValue(sheet.getRow(1).getCell(0)).replace("Dosage", "").replaceAll(" ", "");
			...
			// Resultats à partir de la cellule B81 jusqu'a M88
			for (int i = 80; i < 88; i++) {
				...
				for (int j = 1; j <= 12; j++) {
					...
					String getValue = getStringValue(sheet2.getRow(i).getCell(j));
					if((getValue!=null && !getValue.replaceAll(" ","").equals("<Mini") && !getValue.replaceAll(" ","").equals(">Maxi")) || getValue==null){
						Double concentration1 = getNumericValue(sheet2.getRow(i).getCell(j));
