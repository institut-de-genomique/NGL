...
angular.module('home').controller('SwitchIndexSearchCtrl', ['$scope', '$filter', '$http', 'lists','mainService','tabService', 'messages','datatable',
				...
				this.updateForm();
				 $http.get(jsRoutes.controllers.admin.supports.api.NGLObjects.list().url,{params:this.form})
				 	.success(function(results){
						...
						$scope.searchService.lists.refresh.readSets({projectCode:$scope.searchService.form.projectCode});
				 	}).error(function(data){
	...
	dtConfigBI.columns.push({
		...
		},
		{
			 ...
			 "order":true,
			 "edit":true,
			 "editTemplate":'<div class="form-control" bt-select #ng-model filter="true" placeholder="'+Messages("search.placeholder.readsets")+'" bt-options="readset.code as readset.code for readset in searchService.lists.getReadSets()"></div>',
			 "type":"text",
			 "position":9
			},
		{
