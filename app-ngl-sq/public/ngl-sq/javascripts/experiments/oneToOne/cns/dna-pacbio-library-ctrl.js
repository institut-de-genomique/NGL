angular.module('home').controller('DNAPacBioLibraryCtrl',['$scope', '$parse', '$http', '$filter','atmToSingleDatatable','tagService',
	...
	//Gestion des tag filtre par groupe de tag
	...
	if ( $scope.isNewState() || $scope.isInProgressState() || Permissions.check("admin") ){
		...
		tagService.initTags("index-pacbio-sequencing");
		//$scope.getTags= function(){ return tagService.getAllTags()};
		$scope.lists.refresh.tags({typeCodes:'index-pacbio-sequencing'});
	...
	$scope.selectGroup = {
			...
			select:function(groupName){       	
				...
				if (groupName.value === undefined ){ 
					...
					$scope.lists.refresh.tags({typeCodes:'index-pacbio-sequencing',groupNames:[groupName.value]});
				}
			}
	};
