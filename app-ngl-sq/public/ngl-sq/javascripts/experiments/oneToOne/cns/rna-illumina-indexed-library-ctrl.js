angular.module('home').controller('RNAIlluminaIndexedLibraryCtrl',['$scope', '$parse', '$http', '$filter','atmToSingleDatatable','tagService',
	...
	atmService.convertOutputPropertiesToDatatableColumn = function(property, pName){
		...
		if(property.code=="tag"){
			column.editTemplate='<input class="form-control" type="text" #ng-model typeahead="v.code as v.code for v in lists.getTags() | filter:{groupNames:selectedTagGroup.value} | filter:{code:$viewValue} | limitTo:20" typeahead-min-length=1 udt-change="updatePropertyFromUDT(value,col)"/>';        											
		}
		return column;
	};
	...
	(function initTags() {
		var tagTypeIllumina = "index-illumina-sequencing";
		fetchTags();
		if(isTagGroupsAllowed()) initTagGroups();
		$scope.selectGroup = getSelectGroupObject();

		//---

		function fetchTags() {
			var url = jsRoutes.controllers.commons.api.Parameters.list().url;
			var form = {params: { typeCode: tagTypeIllumina } };
			$http.get(url, form)
		...
		function isTagGroupsAllowed() {
			return $scope.isNewState() || $scope.isInProgressState() || Permissions.check("admin");
		}

		function initTagGroups() {
			tagService.initTags(tagTypeIllumina);
			$scope.getTags = function(){ return tagService.getAllTags(); };
			$scope.getTagGroups = function(){ return tagService.getAllTagGroups(); };
			$scope.selectedTagGroup = getFirstTagGroup();

			//---
	
			function getFirstTagGroup() {
				return $scope.getTagGroups()[0];
			}
		}

		function getSelectGroupObject() {
			return {
				isShow: isTagGroupsAllowed,
				select: function(group) {
					var groupName = group.value;
					console.log( 'groupe choisi :' +  groupName );
					if (groupName === undefined){ 
						$scope.lists.refresh.tags({
							typeCodes: tagTypeIllumina
						});
					} else { 
						$scope.lists.refresh.tags({
							typeCodes: tagTypeIllumina,
