angular.module('home').controller('nanoporeReparationAndEndPrepCtrl',['$scope', '$parse', 'atmToSingleDatatable',
	...
	$scope.updatePropertyFromUDT = function(value, col){
		...
		if (isInputVolume()){
			computeInputQuantityToContentProperties(udtData);
		} else if(isOutputConcentrationOrVolume()) {
			computeOutputQuantity(udtData);
		}

		function isInputVolume() {
			return col.property === 'inputContainerUsed.experimentProperties.inputVolume.value';
		}

		function isOutputConcentrationOrVolume(){
			return col.property === 'outputContainerUsed.concentration.value' || col.property === 'outputContainerUsed.volume.value';
		}
	};
	...
	var computeOutputQuantity = function(udtData) {
		var getter = outputQuantityGetter();
		var quantity = getter(udtData);
		var compute = buildCompute();
		...
		function outputQuantityGetter() {
			return $parse("outputContainerUsed.quantity.value");
		}

		function buildCompute() {
			return {
				volume : getOutputVolume(),
				concentration : getOutputConcentration(),
				isReady:function(){
					return (this.volume && this.concentration);
				}
			};
			...
			function getOutputConcentration() {
				return $parse('outputContainerUsed.concentration.value')(udtData);
			}
		}
		...
		function computeQuantity() {
			var rawValue = getRawQuantity();
			quantity = isCorrectValue() ? getRoundedValue() : undefined;
			setQuantity(quantity);

			//---

			function getRawQuantity() {
				return $parse("(volume * concentration)")(compute);
			}

			function isCorrectValue() {
				return angular.isNumber(rawValue) && !isNaN(rawValue);
			}
		...
		function clearQuantity() {
			...
			console.log("not ready to compute output quantity");
		}
	};
