...
angular.module('home').controller('LibraryPrepCtrl',['$scope', '$parse',  '$filter', 'mainService','atmToSingleDatatable','$http','tagPlates',
                                                     function($scope, $parse, $filter, mainService, atmToSingleDatatable, $http, tagPlates){
	...
	$scope.plates.push( {name:"RAP TruSeq DNA HT (96 Indexes)",                 tagCategory:"DUAL-INDEX", tags: tagPlates.populateIndex_XapTruSeqHT() });
	...
	$scope.selectColOrPlate = {
		...
		select:function(){
			...
			if (($scope.tagPlate.name === "---") && ($scope.tagPlateColumn.name === "---")){
				return tagPlates.unsetTags ($scope.tagPlateToClean, $scope.tagPlateColumnToClean, atmService, $scope.messages)
				...
				if ($scope.tagPlateToClean != undefined){
					tagPlates.unsetTags ($scope.tagPlateToClean, $scope.tagPlateColumnToClean, atmService, $scope.messages)
				}
