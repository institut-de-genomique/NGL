...
angular.module('home').controller('PcrIndexingPurifQmpSeqCtrl',['$scope', '$parse', '$filter', 'atmToSingleDatatable','tagPlates',
                                                        function($scope, $parse, $filter, atmToSingleDatatable, tagPlates){
	...
	$scope.selectColOrPlate = {
		...
		select:function(){
			...
			if ($scope.tagPlate.name === "---"){  // $scope.tagPlateColumn.name ne peut pas etre == "---"
				...
				// NGL-2896 ajout paramètre "auto" pour éviter les warnings inappropriés sur le choix de $scope.tagPlateColumn
				return tagPlates.setTags($scope.tagPlate, $scope.tagPlateColumn, atmService, $scope.messages, "noShowWarning")
			} 
		}
	};
