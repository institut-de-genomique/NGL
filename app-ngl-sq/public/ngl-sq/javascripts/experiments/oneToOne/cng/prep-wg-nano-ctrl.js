...
angular.module('home').controller('PrepWgNanoCtrl',['$scope', '$parse',  '$filter', 'atmToSingleDatatable','$http','tagPlates','tagService',
	...
	// filtrer tags.groupNames sur selectedTagGroup
	...
	atmService.convertOutputPropertiesToDatatableColumn = function(property, pName){
		...
		if(property.code==="tag"){
			...
			// test avec:  in tagService.getAllTags() NON MARCHE PAS !!!!
	...
	if ( $scope.isNewState() || $scope.isInProgressState() || Permissions.check("admin") ){
	   ...
	   tagService.initTags('index-illumina-sequencing' ); // 14/02/2002 filtre ajouté
