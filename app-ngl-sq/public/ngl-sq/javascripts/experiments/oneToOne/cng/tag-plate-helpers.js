...
//03/12/2018 essai de remonté au niveau experiments pour etre communn CNG/CNS... 
...
angular.module('tools',[]).factory('tagPlates', ['$parse', '$filter','$http',
		...
		var factory = {
				...
				// IDT-ILMN TruSeq DNA UD Indexes (24 couples d'indexes DUAL repetes 4 fois !!!)
				...
				// ATTENTION repartition spéciale, pas d'algorithme utilisable !!)
				...
				//QiaSeq miRNA NGS 48 index (48 index Qiagen SINGLE)
				//remplacer par 2 boucles for imbriquées ???
				populateIndex_QiaSeq48:function(){
					var tags=[];
					//			  A         B         C         D         E         F         G         H
				...
				setTags:function(plate, plateColumn, atmService, messages){				
					console.log(">>selected plate :"+ plate.name + "; start column: "+plateColumn.name+ ";start position :"+ plateColumn.position  );	
					messages.clear();
					
					if ((plateColumn.name === '---' ) && (plate.name === '---')){
						// remise a 0 des selects par l'utilisateur ????=> nettoyage de ce qui a été positionné précedemment
						//console.log("suppression des index ...");
					    
						//NEW OK avec utilisation de displayResult et pas besoin de setData(dataMain) => plus rapide ????
						...
						angular.forEach(wells, function(well){
							...
							$parse("experimentProperties.tag.value").assign(outputContainerUsed,null);
							$parse("experimentProperties.tagCategory.value").assign(outputContainerUsed,null);
						});
						
					} else if ((plateColumn.name !== '---' ) && (plate.name !== '---')){			
						...
						// meme en prennant tous les index possibles, il n'y en a pas assez dans la plaque !! (NB il existe des plaques de 48 index)
						// 17/07/2018 pas bloquant donc => warning et pas danger
						if ( lastTagCol < lastInputCol ){
				        	...
				        	//NE PAS BLOQUER, l'utilisateur a la possibilité de compléter manuellement
				        	//return;
						}
						
						// la colonne de début de la plaque d'échantillon choisie est vide (mettre les index dans puits vide INTERDIT)
						if ( plateColumn.name*1 > lastTagCol){	
				        	...
				        	messages.showDetails = false;
				        	messages.open();	
				        	//console.log("alert alert-danger /"+Messages('select.msg.error.emptyStartColumn.tagPlate',plateColumn.name,plate.name));
				        	
				        	return;
				        }
							
						// la colonne choisie est incorrecte (toutes les puits input ne recevront pas d'index)
						// 17/07/2018 pas bloquant donc => warning et pas danger
					    if ( (lastTagCol - plateColumn.name*1  +1) < lastInputCol ) { 
				        	messages.clazz="alert alert-warning";
						...
						angular.forEach(wells, function(well){
							...
							var libPos=(outputContainerUsed.locationOnContainerSupport.column  -1 )*8   + (outputContainerUsed.locationOnContainerSupport.line.charCodeAt(0) -65);
						    var indexPos= libPos + plateColumn.position;
						    console.log("==> setting index "+indexPos+ ": "+ plate.tags[indexPos] ); 
						    $parse("experimentProperties.tag.value").assign(outputContainerUsed,plate.tags[indexPos]);
						    
						    if ( plate.tags[indexPos] === undefined) {
						       // contrairement au cns tagCategory est assigné globalement pour toute la plaque , il faut donc corriger dans certain cas...
							   $parse("experimentProperties.tagCategory.value").assign(outputContainerUsed,null);
						    } else {
						       $parse("experimentProperties.tagCategory.value").assign(outputContainerUsed,plate.tagCategory);
						    }
						});
						//atmService.data.seData(wells);
					}
				},
				...
				// FDS 29/11/2018  ajouter un parametre pour prendre en compte les index Nanopore si besoin 
				...
				initTags: function(type){
					...
					if(angular.isUndefined(type)){
					    types= {typeCodes:['index-illumina-sequencing','index-nanopore-sequencing']};
					} else {
					    //types.typeCodes=[type];
					    types= {typeCodes:[type]};
					}
					
					//$http.get(jsRoutes.controllers.commons.api.Parameters.list().url,{params:{typeCode:['index-illumina-sequencing','index-nanopore-sequencing']}}) // ajout Nanopore !!!!
