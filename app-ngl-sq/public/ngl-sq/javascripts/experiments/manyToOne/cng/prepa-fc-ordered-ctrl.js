angular.module('home').controller('CNGPrepaFlowcellOrderedCtrl',['$scope', '$parse', '$http','atmToDragNDrop','mainService',
	...
	// il doit recharger le fichiercBot pour qu'on puisse garantir la coherence !!
    //-1- stripCode.value
	...
	//function checkNovaSeqInstrument(sequencingType, instrumentCode){	
	function checkNovaSeqInstrument(){	
		...
		//05/09/2018 NGL-2219: ajout du novaseq-xp-fc-dock
		if ( ! sequencingType.match(/NovaSeq 6000/) && ( instrumentCode.match(/MarieCurix/) || instrumentCode.match(/novaseq-xp-fc-dock/)) ){
			setAlert("Alertes","Le type de séquencage doit etre NovaSeq 6000 / * .");	
		} 
		
		//ajouter la verification inverse; 05/09/2018 NGL-2219: ajout du novaseq-xp-fc-dock
		if ( sequencingType.match(/NovaSeq 6000/)  && ( !instrumentCode.match(/MarieCurix/)  && !instrumentCode.match(/novaseq-xp-fc-dock/)) ){
			setAlert("Alertes","L'instrument choisi n'est pas compatible avec un séquençage sur NovaSeq");
		} 
	}
	...
	function checkXPSequencing(){
		...
		console.log( "checkOnboardXP..."+sequencingType +"/"+instrumentCode)
		
		if ( sequencingType.match(/XP/) && 
			( ! instrumentCode.match(/MarieCurix/) && ! instrumentCode.match(/novaseq-xp-fc-dock/)) ){
	...
	function checkFcPattern (){	
		...
		// 17/12/2018 NGL-2364 Illumina va remplacer les barcodes CCXY par CCX2==> gerer les 2
		var HXfcRegexp= /^[A-Za-z0-9]*CCX[Y2]$/;
		
		//var Nv6000SPfcRegexp= /^[A-Za-z0-9]*????$/; // attente spec pour pattern FC SP....
		var Nv6000S1fcRegexp= /^[A-Za-z0-9]*DRXX$/; // spec  30/07/2018
		...
		if ((sequencingType === "Hiseq 4000") && ( null == fcBarcode.match(H4000fcRegexp))) {
			setAlert("Alertes", "Code Flowcell n'est pas du type 'Hiseq 4000' (*BB--).");
		...
		// NGL-2624 renommage "S1" => "S1-SP" mais pas de spec pour pattern FC pour l'instant...
		/*else if (((sequencingType === "NovaSeq 6000 / S1-SP")||
				    (sequencingType === "NovaSeq 6000 / S1-SP / XP")) && 
				   (( null == fcBarcode.match(Nv6000S1fcRegexp)) && ( null == fcBarcode.match(Nv6000SPfcRegexp)))) { 
			setAlert("Alertes","Le Code Flowcell n'est ni du type 'NovaSeq 6000 / S1' (*DRXX) ni du type 'NovaSeq 6000 / SP (*????) '");
		*/
		  else if (((sequencingType === "NovaSeq 6000 / S1-SP")||
			        (sequencingType === "NovaSeq 6000 / S1-SP / XP")) &&  ( null == fcBarcode.match(Nv6000S1fcRegexp))) {
			setAlert("Alertes","Le Code Flowcell n'est pas du type 'NovaSeq 6000 / S1' (*DRXX) [ si Flowcell de type  'NovaSeq 6000 / SP' ne pas tenir compte de cette alerte ]");
					   
		} else if (((sequencingType === "NovaSeq 6000 / S2")||
				    (sequencingType === "NovaSeq 6000 / S2 / XP")) && (null == fcBarcode.match(Nv6000S2fcRegexp))) {
			setAlert("Alertes","Le Code Flowcell n'est pas du type 'NovaSeq 6000 / S2' (*DMXX).");
	
		} else if (((sequencingType === "NovaSeq 6000 / S4")||
				    (sequencingType === "NovaSeq 6000 / S4 / XP")) && (null == fcBarcode.match(Nv6000S4fcRegexp))) {
			setAlert("Alertes","Le Code Flowcell n'est pas du type 'NovaSeq 6000 / S4' (*DSXX).");
		}
	}
	...
	function checkNbLaneCompat(){
		...
		if        (((sequencingType === "Hiseq 4000") || (sequencingType === "Hiseq X")) && ( outContainerSupportCategoryCode != "flowcell-8")) {
			setAlert("Alertes", "Le type de séquencage "+ sequencingType +" nécessite une flowcell 8 pistes.");
				    ...
				    (sequencingType === "NovaSeq 6000 / S1-SP/ XP")||
				    (sequencingType === "NovaSeq 6000 / S2")||
				    (sequencingType === "NovaSeq 6000 / S2 / XP")) && (outContainerSupportCategoryCode != "flowcell-2")){ 
	...
	function checkNbLaneCompat2(){
		...
		// puisqu'il existe pour Hiseq, NextSeq, MiSeq et NovaSeq !!!!!!
		// separer de checkNbLaneCompat pour fonctionner sans reference a sequencingType
		console.log("checkNbLaneCompat2...");
		...
		instrumentTypecode=$scope.experiment.instrument.typeCode;
		instrumentCode=$scope.experiment.instrument.code;
		console.log("checkNbLaneCompat2..."+instrumentCode+"/"+outContainerSupportCategoryCode)
		...
		if ( instrumentTypecode === "cBot-onboard"  ){
			if ( instrumentCode.match(/Miseq/) && outContainerSupportCategoryCode != "flowcell-1" ){          // !!! (/Miseq/) avec "s"
				...
				setAlert("Alertes","L'instrument Nextseq n'accepte que la flowcell 4 pistes.");
				
			}
			/*11/02/2019 J. Guy ne veut pas de ce message....
			else if ( instrumentCode.match(/MarieCurix/) && outContainerSupportCategoryCode === "flowcell-1"){
				setAlert("Alertes","L'instrument NovaSeq 6000 n'accepte pas la flowcell 1 piste.")                //     NovaSeq avec "S"
			}
			*/
		}
	}
