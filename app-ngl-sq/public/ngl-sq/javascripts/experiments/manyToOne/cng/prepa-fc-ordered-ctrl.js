angular.module('home').controller('CNGPrepaFlowcellOrderedCtrl',['$scope', '$parse', '$http','atmToDragNDrop','mainService',
	...
	$scope.buttonMettler = {
		isShow:function(){
			...
			// 13/11/2019 NGL-2752 ajout SuperNova
			return ( ( $scope.isInProgressState()||$scope.isFinishState()) && 
					                 ((null===$scope.experiment.instrument.code.match(/MarieCurix/))||(null===$scope.experiment.instrument.code.match(/SuperNova/))) );
			},
		isFileSet:function(){
			return ($scope.file == undefined)?"disabled":"";
		},
	...
	function checkNovaSeqInstrument(){	
		...
		//ajouter la verification inverse; 05/09/2018 NGL-2219: ajout du novaseq-xp-fc-dock; 14/11/2019 NGL-2752 ajout /SuperNova/
		//18/11/2019 NGL-2752 utiliser isInstrumentNovaSeq6000()
		if ( sequencingType.match(/NovaSeq 6000/) && ! isInstrumentNovaSeq6000()) {
	...
	function checkXPSequencing(){
		...
		// 14/11/2019 NGL-2752 ajout SuperNova; 18/11/2019 utilisation isInstrumentNovaSeq6000()
		if ( sequencingType.match(/XP/) && ! isInstrumentNovaSeq6000()){
			//( ! instrumentCode.match(/MarieCurix/) && ! instrumentCode.match(/SuperNova/) && ! instrumentCode.match(/novaseq-xp-fc-dock/)) ){
	...
	function checkFcPattern (){	
		...
		// NGL-2624 renommage "S1" => "S1-SP" mais pas de spec pour pattern FC pour l'instant...voir au dessus
