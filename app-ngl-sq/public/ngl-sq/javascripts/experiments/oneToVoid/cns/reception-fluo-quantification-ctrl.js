	...
	var helpers = {
			...
			computeCircuMgiRatioSs : function(udtData){
				...
				var compute = {
						...
						isReady:function(){
							return (this.conc1 && this.volume1 && this.ssQtty && this.fromTransfoType === "mgi-sscircularisation");
						}
				};
				
				if(compute.isReady()){
					var result = $parse("((conc1*volume1) / ssQtty * 100)")(compute);
					if(angular.isNumber(result) && !isNaN(result)){
						ssRatio=result;
					}else{
						ssRatio = undefined;
			...
			hideColumns:function(atmService, visualChoice){
				...
				for(var i = 13; i <= 35 ; i++){
					...
					// BR prop i = 13,14,15,16	
					// HS1 prop i = 17,18,19,20
					//HS2 prop i = 21, 22,23,24
					//HS3 prop i = 25, 26,27,28
					//prop communes dosage fluo 29 à 33 
					//prop spécifiques MGI 34 à 35
					...
					if(selectedColumns.length === 1){
						...
						//udt.isHide(column.id)
						else if(("Tout" === visualChoice && 13 <= i && i <= 28 
								|| "BR1 + HS1 + HS2"  === visualChoice && 13 <= i && i <= 24
								|| "HS1 + HS2 + HS3"  === visualChoice && 17 <= i && i <= 28 )
								&& udt.isHide(column.id)){
							udt.setHideColumn(column);
						}else if ( ("HS1 (single stranded) (MGI)" === visualChoice && 17 <= i && i <= 20
								||  "HS1" === visualChoice && 17 <= i && i <= 20)
								&& udt.isHide(column.id)){
							udt.setHideColumn(column);
						}
						//!udt.isHide(column.id)
						else if( ("BR1 + HS1 + HS2"  === visualChoice && i > 24
								|| "HS1 + HS2 + HS3"  === visualChoice && i < 17)
								&& !udt.isHide(column.id) ){
							udt.setHideColumn(column);
						}else if ( ("HS1 (single stranded) (MGI)" === visualChoice && i < 17 
								|| "HS1 (single stranded) (MGI)" === visualChoice && 20 < i  
								|| "HS1" === visualChoice  &&i < 17 
								|| "HS1" === visualChoice  &&  20 < i)
								&& !udt.isHide(column.id)){
							udt.setHideColumn(column);								
						}else{
