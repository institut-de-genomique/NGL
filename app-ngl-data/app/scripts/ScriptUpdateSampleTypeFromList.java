...
public class ScriptUpdateSampleTypeFromList extends ScriptWithExcelBody{
	...
	public void execute(XSSFWorkbook workbook) throws Exception {
			...
			sheet.rowIterator().forEachRemaining(row -> {
				...
				if(row!=null && row.getCell(0)!=null && row.getCell(1)!=null){
					...
					if(!sampleCode.equals("")){
						//update Sample
						updateSample(sampleCode, oldSampleTypeCode, newSampleTypeCode,oldSampleCategoryCode,newSampleCategoryCode, cv);
						//update containers
		...
		Logger.debug("End Update SampleType and SampleTypeCatgory "+new Date());
	}
	@SuppressWarnings("unchecked")
	private void updateSample(String sampleCode, String oldSampleTypeCode, String newSampleTypeCode, String oldSampleCategoryCode, String newSampleCategoryCode, ContextValidation cv)
	{
		...
		for(Sample child : samples){
			...
			((List<String>)cv.getObject("RecapSample")).add(child.code+","+InstanceConstants.SAMPLE_COLL_NAME+",life.from.sampleTypeCode"+newSampleTypeCode);
