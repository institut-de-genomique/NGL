...
public class Workflows {
	...
	@Inject
	public Workflows(NGLApplication app) {
		rulesActor = app.actorSystem().actorOf(Props.create(RulesActor6.class));
		rulesKey   = app.nglConfig().getRulesKey();
	}
	...
	public static void nextRunState(ContextValidation contextValidation, Run run) {
		State nextStep = State.cloneState(run.state);
	...
	private static void applyRunRules(ContextValidation contextValidation, Run run) {
		if("F-RG".equals(run.state.code)){
			...
			for(ReadSet readSet: readSets){
				State nextReadSetState = State.cloneState(run.state);
...
//			rulesActor.tell(new RulesMessage(Play.application().configuration().getString("rules.key"),ruleStatRG,run),null);
	...
	private static void applyReadSetRules(ContextValidation contextValidation, ReadSet readSet) {
		if ("F-RG".equals(readSet.state.code)) {
			...
			State state = State.cloneState(readSet.state);
	...
	public static void nextReadSetState(ContextValidation contextValidation, ReadSet readSet) {
		State nextStep = State.cloneState(readSet.state);
		if("F-RG".equals(readSet.state.code)){
			nextStep.code = "IW-QC";
...
//			return (StringUtils.isNotBlank(p.bioinformaticParameters.regexBiologicalAnalysis))?readSet.code.matches(p.bioinformaticParameters.regexBiologicalAnalysis):p.bioinformaticParameters.biologicalAnalysis; // TO DO matche PE of type F
//			
//		}
//		return false;
//	}
	private static boolean isHasBA(ReadSet readSet){
		Project p = MongoDBDAO.findByCode(InstanceConstants.PROJECT_COLL_NAME, Project.class, readSet.projectCode);
		if (p.bioinformaticParameters.biologicalAnalysis) {
			if (StringUtils.isNotBlank(p.bioinformaticParameters.regexBiologicalAnalysis))
				return readSet.code.matches(p.bioinformaticParameters.regexBiologicalAnalysis);
			return p.bioinformaticParameters.biologicalAnalysis; // GA: matche PE of type F
		}
		return false;
	}
	...
	private static models.laboratory.common.description.State getStateDescription(
			State state) {
		try {
			return models.laboratory.common.description.State.find.get().findByCode(state.code);
...
//	}
	...
	public static void nextAnalysisState(ContextValidation contextValidation, Analysis analysis) {
		State nextStep = State.cloneState(analysis.state);
		if ("IP-BA".equals(analysis.state.code)) {
