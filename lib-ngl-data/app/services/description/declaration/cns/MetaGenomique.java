...
public class MetaGenomique extends AbstractDeclaration {
	...
	protected List<ExperimentType> getExperimentTypePROD() {
     ...
     protected List<ExperimentType> getExperimentTypeCommon() {
		...
		l.add(newExperimentType("Ext to PCR Free from frag","ext-to-pcr-free-process-from-frag",null,-1,
				ecfind.findByCode(ExperimentCategory.CODE.voidprocess.name()), null, null,"OneToOne",
                DescriptionFactory.getInstitutes(Constants.CODE.CNS)));
	...
	protected List<ProcessType> getProcessTypeUAT() {
     ...
     protected List<ProcessType> getProcessTypeCommon() {
             ...
             l.add(DescriptionFactory.newProcessType("(Meta)Génomique ou reprise à partir cDNA", "metagenomic-process", pcfind.findByCode("library"), 31,
             ...
             l.add(DescriptionFactory.newProcessType("(Meta)Génomique (spri select)", "metagenomic-process-with-spri-select", pcfind.findByCode("library"), 32,
             ...
             l.add(DescriptionFactory.newProcessType("(Meta)Génomique avec sizing (gel)", "metagenomic-process-with-sizing", pcfind.findByCode("library"), 33,
             ...
             l.add(DescriptionFactory.newProcessType("PCR Free", "pcr-free-process", pcfind.findByCode("library"), 34,
             ...
             l.add(DescriptionFactory.newProcessType("PCR Free (reprise à partir de frag)", "pcr-free-process-from-frag", pcfind.findByCode("library"), 35,
                     getPropertiesPCRFree(),
                     Arrays.asList(getPET("ext-to-pcr-free-process-from-frag",-1)
                                     ...
                                     getExperimentTypes("dna-illumina-indexed-library").get(0), getExperimentTypes("illumina-depot").get(0), getExperimentTypes("ext-to-pcr-free-process-from-frag").get(0),
                                     DescriptionFactory.getInstitutes(Constants.CODE.CNS)));
	...
	private List<PropertyDefinition> getPropertiesMetaGenomique() {
		//liste des type de processus de banque LibProcessDA + banqueProcessTypeMetaRA + (NGL-2717) libProcessHi-C
		List<Value> libProcessTypeCodes = getLibProcessDA();
		libProcessTypeCodes.addAll(MetaTProcess.getBanqueProcessTypeMetaRA());
		libProcessTypeCodes.addAll( getLibProcessHiC());
		List<PropertyDefinition> propertyDefinitions = new ArrayList<>();
		propertyDefinitions.add(newPropertiesDefinition("Type processus Banque", "libProcessTypeCode", LevelService.getLevels(Level.CODE.Process,Level.CODE.Content), String.class, true, null, 
				libProcessTypeCodes, null,null,null,"single", 13, true, null, null));
		propertyDefinitions.add(newPropertiesDefinition("Protocole banque DNA", "dnaLibraryProtocol", LevelService.getLevels(Level.CODE.Process), String.class, true, null, 
				DescriptionFactory.newValues("NEB Ultra 2","low cost","super low cost","autre","Swift Accel 1S","Hi-C","A définir"), null,null,null,"single", 14, true, null, null));
	...
	protected void getExperimentTypeNodeCommon() {
		save(newExperimentTypeNode("ext-to-metagenomic-process", AbstractExperimentService.getExperimentTypes("ext-to-metagenomic-process").get(0), false, false, false, null, null, null, null));
		save(newExperimentTypeNode("ext-to-metagenomic-process-with-sizing", AbstractExperimentService.getExperimentTypes("ext-to-metagenomic-process-with-sizing").get(0), false, false, false, null, null, null, null));
		save(newExperimentTypeNode("ext-to-metagenomic-process-with-spri-select", AbstractExperimentService.getExperimentTypes("ext-to-metagenomic-process-with-spri-select").get(0), false, false, false, null, null, null, null));
		save(newExperimentTypeNode("ext-to-pcr-free-process", AbstractExperimentService.getExperimentTypes("ext-to-pcr-free-process").get(0), false, false, false, null, null, null, null));
		save(newExperimentTypeNode("ext-to-pcr-free-process-from-frag", AbstractExperimentService.getExperimentTypes("ext-to-pcr-free-process-from-frag").get(0), false, false, false, null, null, null, null));
	}
	...
	public static  List<Value> getLibProcessHiC() {
		List<Value> values = new ArrayList<>();
