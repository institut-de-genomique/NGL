...
public class DescriptionFactory {
    ...
    // TODO AJ: code reactivé suite au merge a3d45fd387e48e77e1eca5a995c87d66267a3521
	private static final play.Logger.ALogger logger = play.Logger.of(DescriptionFactory.class);
	
//	public static <T extends AbstractCategory<T>> T newSimpleCategory(Class<T> classCat, String name, String code){
	
	/**
	 * Create an instance of the provided category subclass with the provided name and code.
	 * @param classCat category subclass
	 * @param name     category name
	 * @param code     category code
	 * @return         category instance
	 * @deprecated use newXXXCategory specialized method (e.g: {@link #newProcessCategory(String, String)})
	 */
    // TODO AJ: code reactivé suite au merge a3d45fd387e48e77e1eca5a995c87d66267a3521
	@Deprecated
	public static <T extends AbstractCategory> T newSimpleCategory(Class<T> classCat, String name, String code) {
		try {
	...
	public static ProcessCategory newProcessCategory(String name, String code) {
		ProcessCategory c = new ProcessCategory();
		c.name = name;
		c.code = code;
		return c;
	}
...
//		pd.name = name;
		...
		if (editable == null) editable = Boolean.TRUE;
...
//			                                                 int displayOrder) throws DAOException {
//		PropertyDefinition pd = new PropertyDefinition();		
////		pd.name              = name;
//		pd.setName(name);
//		pd.code              = code;
//		// pd.active            = true; // default value in the class definition
//		pd.levels            = levels;
//		pd.editable          = editable;
//		pd.valueType         = type.getName();
//		pd.required          = required;
//		// pd.choiceInList      = false; // default value in the class definition
//		pd.propertyValueType = propertyValueType;
//		pd.displayOrder      = displayOrder;
//		return pd;
//	}
...
////				value.defaultValue = false;
////			}
//			value.defaultValue = v.equals(defaultValue);
//			l.add(value);
//		}
//		return l;
//	}
...
//		List<Value> l = new ArrayList<>(code.length);
//		for (String v : code) 
//			l.add(newValue(v, v, v.equals(defaultValue)));
//		return l;
//	}
	
	public static List<Value> newValuesWithDefault(String defaultValue, String... codes) {
		return Iterables.zenThem(codes)
				.map(v -> newValue(v, v, v.equals(defaultValue)))
				.toList();
	}
...
//	}
	...
	public static ObjectType setStatesToObjectType(String code, Boolean generic, List<State> states) {
		ObjectType l = new ObjectType();
		try {
			l = ObjectType.find.get().findByCode(code);
...
//	}
	...
	public static MeasureUnit newMeasureUnit(String code, String value, boolean defaultUnit, MeasureCategory category) {
...
//		ContainerSupportCategory csc = DescriptionFactory.newSimpleCategory(ContainerSupportCategory.class, name, code);
		ContainerSupportCategory csc = new ContainerSupportCategory(code,name);
		csc.nbLine = nbLine;
	...
	public static ExperimentType newExperimentType(String name, String code, String shortCode, Integer displayOrder, ExperimentCategory category, List<PropertyDefinition> propertiesDefinitions, List<InstrumentUsedType> instrumentUsedTypes,String atomicTransfertMethod, List<SampleType> sampleTypes, Boolean active, List<Institute> institutes) throws DAOException {
		...
		if (shortCode != null) et.shortCode = shortCode.toUpperCase();
		et.name = name;
		et.displayOrder = displayOrder;
		et.category = category;
		et.objectType = ObjectType.find.get().findByCode(ObjectType.CODE.Experiment.name());
		et.propertiesDefinitions = propertiesDefinitions;
		//et.protocols = protocols;
		et.instrumentUsedTypes = instrumentUsedTypes;
		et.states = State.find.get().findByObjectTypeCode(ObjectType.CODE.Experiment);
...
//	}
	...
	public static InstrumentUsedType newInstrumentUsedType(String name, String code, InstrumentCategory category, List<PropertyDefinition> propertiesDefinitions, List<Instrument> instruments,
		 ...
		 return newInstrumentUsedType(name, code, category, propertiesDefinitions, instruments, inContainerSupportCategories, outContainerSupportCategories, institutes, Boolean.TRUE);
	 }
	
	public static InstrumentUsedType newInstrumentUsedType(String name, String code, InstrumentCategory category, List<PropertyDefinition> propertiesDefinitions, List<Instrument> instruments,
			List<ContainerSupportCategory> inContainerSupportCategories, List<ContainerSupportCategory> outContainerSupportCategories, List<Institute> institutes, Boolean active) throws DAOException{
		InstrumentUsedType iut = new InstrumentUsedType();
		iut.code =code;
		iut.name =name;
		iut.category = category;
		iut.objectType = ObjectType.find.get().findByCode(ObjectType.CODE.Instrument.name());
		...
		iut.states = State.find.get().findByObjectTypeCode(ObjectType.CODE.Instrument);
		iut.active = active;
		return iut; 
	}
	...
	public static ProcessType newProcessType(String name, 
			                                 String code, 
			                                 ProcessCategory category, 
			                                 Integer displayOrder, 
			                                 List<PropertyDefinition> propertiesDefinitions, 
			                                 List<ProcessExperimentType> experimentTypes, 
			                                 ExperimentType firstExperimentType, 
			                                 ExperimentType lastExperimentType, 
			                                 ExperimentType voidExperimentType, 
		...
		pt.objectType            = ObjectType.find.get().findByCode(ObjectType.CODE.Process.name());
		pt.propertiesDefinitions = propertiesDefinitions;
		pt.states                = State.find.get().findByObjectTypeCode(ObjectType.CODE.Process);
	...
	public static SampleType newSampleType(String name, String code, SampleCategory category, List<PropertyDefinition> propertiesDefinitions, List<Institute> institutes) throws DAOException{
		...
		st.objectType            = ObjectType.find.get().findByCode(ObjectType.CODE.Sample.name());
		st.propertiesDefinitions = propertiesDefinitions;
		st.institutes            = institutes;
		st.states                = State.find.get().findByObjectTypeCode(ObjectType.CODE.Sample); 
		return st;
	}
	...
	public static ImportType newImportType(String name, String code, ImportCategory category, List<PropertyDefinition> propertiesDefinitions, List<Institute> institutes) throws DAOException{
		...
		it.objectType = ObjectType.find.get().findByCode(ObjectType.CODE.Import.name());
		it.propertiesDefinitions = propertiesDefinitions;
		it.institutes = institutes;
		it.states = State.find.get().findByObjectTypeCode(ObjectType.CODE.Sample); 
		return it;
	}
	...
	public static ProjectType newProjectType(String name, String code, ProjectCategory category, List<PropertyDefinition> propertiesDefinitions, List<Institute> institutes) throws DAOException{
		...
		pt.name = name;
		pt.category = category;
		pt.objectType = ObjectType.find.get().findByCode(ObjectType.CODE.Project.name());
		pt.propertiesDefinitions = propertiesDefinitions;
		pt.institutes = institutes;
		pt.states = State.find.get().findByObjectTypeCode(ObjectType.CODE.Project);
		return pt;
	}
	...
	public static ReadSetType newReadSetType(String name, String code, List<PropertyDefinition> propertiesDefinitions, List<Institute> institutes) throws DAOException {
		...
		rt.code =code;
		rt.name =name;
		rt.objectType = ObjectType.find.get().findByCode(ObjectType.CODE.ReadSet.name());
		rt.propertiesDefinitions = propertiesDefinitions;
		rt.states = State.find.get().findByObjectTypeCode(ObjectType.CODE.ReadSet);
	...
	public static AnalysisType newAnalysisType(String name, String code, List<PropertyDefinition> propertiesDefinitions, List<Institute> institutes) throws DAOException {
		...
		rt.code                  = code;
		rt.name                  = name;
		rt.objectType            = ObjectType.find.get().findByCode(ObjectType.CODE.Analysis.name());
		rt.propertiesDefinitions = propertiesDefinitions;
		rt.states                = State.find.get().findByObjectTypeCode(ObjectType.CODE.Analysis);
		rt.institutes            = institutes; 
	...
	public static RunType newRunType(String name, String code, Integer nbLanes, RunCategory category, List<PropertyDefinition> propertiesDefinitions, List<Institute> institutes) throws DAOException {
		...
		rt.objectType = ObjectType.find.get().findByCode(ObjectType.CODE.Run.name());
		rt.propertiesDefinitions = propertiesDefinitions;
		rt.states = State.find.get().findByObjectTypeCode(ObjectType.CODE.Run);
		rt.institutes = institutes;
	...
	public static TreatmentType newTreatmentType(String name, String code, TreatmentCategory category, String names, List<PropertyDefinition> propertiesDefinitions, List<TreatmentTypeContext>  contexts, List<Institute> institutes, String displayOrders) throws DAOException {
		...
		tt.objectType            = ObjectType.find.get().findByCode(ObjectType.CODE.Treatment.name());
	...
	public static List<State> getStates(String...codes) throws DAOException {
		...
		for(String code: codes){
			states.add(State.find.get().findByCode(code));
		}
		return states;
	}
	...
	public static List<ObjectType> getObjectTypes(String...codes) throws DAOException {
		...
		for(String code: codes){
			objectTypes.add(ObjectType.find.get().findByCode(code));
		}
		return objectTypes;
	}
	...
	public static HashMap<ObjectType, State> getHierarchies(String...hierarchies)  throws DAOException {
		...
		for (String hierarchy: hierarchies) {
			...
			hm.put(ObjectType.find.get().findByCode(objectTypeCode), State.find.get().findByCode(stateCode));
		}
		return hm;
