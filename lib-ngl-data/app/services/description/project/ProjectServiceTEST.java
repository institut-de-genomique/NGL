...
//		l.add(DescriptionFactory.newSimpleCategory(ProjectCategory.class,"defaut", "default"));
	...
	public void saveProjectTypes(Map<String, List<ValidationError>> errors) throws DAOException{
		...
		l.add(DescriptionFactory.newProjectType("Defaut", "default-project", ProjectCategory.find.get().findByCode("default"), null, DescriptionFactory.getInstitutes(Constants.CODE.TEST)));
		l.add(DescriptionFactory.newProjectType("France Génomique", "france-genomique", ProjectCategory.find.get().findByCode("default"), getFGPropertyDefinitions(), DescriptionFactory.getInstitutes(Constants.CODE.TEST)));
