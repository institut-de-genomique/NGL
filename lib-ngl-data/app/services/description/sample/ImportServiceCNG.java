...
//		l.add(newImportCategory("Sample Import", "sample-import"));
	...
	public List<ImportType> getImportTypes() {
		...
		l.add(newImportType("Import librairies (poolées) SANS démultiplexage", "no-demultiplexing-lib-pool-reception",icfind.findByCode("sample-import"), getLibraryNodemultiplexReceptionPropertyDefinitions(),CNG));
	...
	// Ajouter les memes valeurs a l'identique dans ProcessServiceCNG si necessaire et pls bas dans getExtLibProcessTypecodesValues si necessaire
	private static List<Value> getLibProcessTypeCodeValues(){
        List<Value> values = new ArrayList<>();
         ...
         //   en cas d'ajout, ajouter aussi a l'identique dans ProcessServiceCNG/getCaptureLibProcessTypeCodeValues
	...
	private static List<Value> getExtLibProcessTypecodesValues(){
        ...
        // 04/07/2017 ajout des codes pour certaines librariries externes RNA
        values.add(DescriptionFactory.newValue("RA","RA - RNASeq"));
        values.add(DescriptionFactory.newValue("RB","RB - smallRNASeq"));
        values.add(DescriptionFactory.newValue("RC","RC - ssRNASeq"));
        values.add(DescriptionFactory.newValue("RD","RD - ssmRNASeq"));
