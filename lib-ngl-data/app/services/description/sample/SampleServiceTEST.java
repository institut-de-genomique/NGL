...
//		return l;
	...
	public List<SampleType> getSampleTypes() {
		...
		SampleCategoryDAO scfind = SampleCategory.find.get();
		List<Institute> TEST = DescriptionFactory.getInstitutes(Constants.CODE.TEST);
		
		l.add(newSampleType("BAC",      "BAC",     scfind.findByCode("cloned-DNA"), getPropertyDefinitionsADNClone(), TEST));	
		l.add(newSampleType("Plasmide", "plasmid", scfind.findByCode("cloned-DNA"), getPropertyDefinitionsADNClone(), TEST));	
		l.add(newSampleType("Fosmide",  "fosmid",  scfind.findByCode("cloned-DNA"), getPropertyDefinitionsADNClone(), TEST));	
		
		// ajout CNG
		l.add(newSampleType("ADN Génomique",     "gDNA",     scfind.findByCode("DNA"), getPropertyDefinitionsADNGenomic(), TEST)); 
		l.add(newSampleType("ADN Métagénomique", "MeTa-DNA", scfind.findByCode("DNA"), getPropertyDefinitionsADN(),        TEST));	
		
		l.add(newSampleType("Amplicon", "amplicon",       scfind.findByCode("amplicon"), getPropertyDefinitionsAmplicon(), TEST));
		
		l.add(newSampleType("ARN total",   "total-RNA",   scfind.findByCode("RNA"), getPropertyDefinitionsARN(), TEST));
		l.add(newSampleType("ARNm",        "mRNA",        scfind.findByCode("RNA"), getPropertyDefinitionsARN(), TEST));
		l.add(newSampleType("Small RNA",   "sRNA",        scfind.findByCode("RNA"), getPropertyDefinitionsARN(), TEST));
		l.add(newSampleType("ARN déplété", "depletedRNA", scfind.findByCode("RNA"), getPropertyDefinitionsARN(), TEST));
		l.add(newSampleType("aARN",        "aRNA",        scfind.findByCode("RNA"), getPropertyDefinitionsARN(), TEST));		
		...
		l.add(newSampleType("ADN en plug", "DNAplug", scfind.findByCode("DNAplug"), getSampleDefinitionADNplug(), TEST));
		l.add(newSampleType("cDNA", "cDNA", scfind.findByCode("cDNA"), getPropertyDefinitionscDNA(), TEST));	

		l.add(newSampleType("ChIP", "chIP", scfind.findByCode("IP-sample"), getSampleCommonPropertyDefinitions(), TEST));
		l.add(newSampleType("ClIP", "clIP", scfind.findByCode("IP-sample"), getSampleCommonPropertyDefinitions(), TEST));
		...
		l.add(newSampleType("Inconnu", "unknown", scfind.findByCode("unknown"), getSampleCommonPropertyDefinitions(), TEST));
		
		//use only in  NGL-BI. Please not used in sample import !!!!!!!!!!!!!!!
		l.add(newSampleType("Non défini", "not-defined", scfind.findByCode("unknown"),null, TEST));
	...
	private static List<PropertyDefinition> getSampleCommonPropertyDefinitions() throws DAOException {
		...
		propertyDefinitions.add(newPropertiesDefinition("Taille associée au taxon", "taxonSize", LevelService.getLevels(Level.CODE.Content,Level.CODE.Sample),Long.class, true,MeasureCategory.find.get().findByCode(MeasureService.MEASURE_CAT_CODE_SIZE), mufind.findByCode("pb"), mufind.findByCode("pb"), "single"));
	...
	public static List<PropertyDefinition> getPropertyDefinitionsADNGenomic() throws DAOException {
        ...
        // GA: GCpercent same as TreatmentService
	...
	public static List<PropertyDefinition> getPropertyDefinitionsARN() throws DAOException {
		MeasureUnitDAO     mufind = MeasureUnit.find.get();
		List<PropertyDefinition> propertyDefinitions = new ArrayList<>();
		propertyDefinitions.add(newPropertiesDefinition("Taille associée au taxon", "taxonSize", LevelService.getLevels(Level.CODE.Sample, Level.CODE.Content),Long.class, true,MeasureCategory.find.get().findByCode(MeasureService.MEASURE_CAT_CODE_SIZE), mufind.findByCode("pb"),mufind.findByCode("pb"), "single"));
		propertyDefinitions.add(newPropertiesDefinition("Fragmenté", "isFragmented", LevelService.getLevels(Level.CODE.Sample),Boolean.class, true, "single"));
		propertyDefinitions.add(newPropertiesDefinition("Code LIMS", "limsCode", LevelService.getLevels(Level.CODE.Sample),Integer.class, false, "single"));
		return propertyDefinitions;
	}
