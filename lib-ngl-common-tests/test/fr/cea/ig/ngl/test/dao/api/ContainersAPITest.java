...
public class ContainersAPITest {
	...
	private static final CC2<Container, Container> containerResource = 
			TUResources.createFullContainerOnTubeResource
	...
	public void updateTest() throws Exception {
		...
		usingData.accept((refContainer, data) -> {
			...
			contToUpdate.traceInformation.modifyUser = TUResources.USER;
			contToUpdate.traceInformation.modifyDate = new Date();
			Container cont = api.get().update(contToUpdate, TUResources.USER);
	...
	public void changeCategoryCodeTest() throws Exception {
		...
		usingData.accept((refContainer, data) -> {
			...
			assertThrows(APIValidationException.class, () -> api.get().update(contToUpdate, TUResources.USER));
		});
	}
	...
	public void updateFieldsTest() throws Exception {
		...
		usingData.accept((refContainer, data) -> {
			...
			Container cont = api.get().update(contToUpdate, TUResources.USER, Arrays.asList("volume", "concentration"));
	...
	public void updateStateTest() throws Exception {
		...
		usingData.accept((refContainer, data) -> {
			State state = new State(ContainerStateNames.IS, TUResources.USER);
			Container cont = api.get().updateState(refContainer.code, state, TUResources.USER);
