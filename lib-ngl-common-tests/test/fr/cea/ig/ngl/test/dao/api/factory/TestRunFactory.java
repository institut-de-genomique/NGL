...
//	private static final play.Logger.ALogger logger = play.Logger.of(TestRunFactory.class);
...
//        return run(user, new HashSet<>(Arrays.asList(projCode)), new HashSet<>(Arrays.asList(sampleCode)), supportCode, instrument, seqDate);
        return run(user, Collections.singleton(projCode), Collections.singleton(sampleCode), supportCode, instrument, seqDate);
    }
...
//        form.projectCode = projCode;
        form.projectCodes = Arrays.asList(projCode);
