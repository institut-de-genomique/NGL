...
public class ReadSetFilesAPITest {
    ...
    public static final CC3<TestContext, ReadSet, ReadSet> illuminaData = TUResources.createIlluminaRunAndReadSetRWC
    ...
    public void illumina_checkObjectExistTest() throws Exception {
        illuminaData.accept((context, refReadSet, readSet) -> {
            ...
            File input = TestReadsetFactory.rawFile();
            api.get().save(readSet, input, TUResources.USER);
    ...
    public void deleteTest() throws Exception {
        illuminaDataWithFile.accept((context, readSet, file) -> {
            api.get().delete(readSet, file.fullname, TUResources.USER);
    ...
    public void deleteByRunCodeTest() throws Exception {
        TUResources.createIlluminaRunAndReadSetRWC.accept((__, ___, ____, run, refReadSet, readSet) -> {
    ...
    public void saveTest() throws Exception {
        illuminaData.accept((context, refReadSet, readSet) -> {
            ...
            File input = TestReadsetFactory.rawFile();
            File file = api.get().save(readSet, input, TUResources.USER);
            readSet = readSetApi.get().get(readSet.code);
    ...
    public void updateTest() throws Exception {
        illuminaDataWithFile.accept((context, readSet, file) -> {
            ...
            File newFile = api.get().update(readSet, file, TUResources.USER);
...
//            assertEquals(val, (String)newFile.properties.get(key).value);
    ...
    public void updateFieldsTest() throws Exception {
        illuminaDataWithFile.accept((context, readSet, file) -> {
            ...
            File newFile = api.get().update(readSet, input, TUResources.USER, Arrays.asList("fullname"), file.fullname);
