...
//		ListFormWrapper<Sample> othering = TestSampleFactory.wrapper(refProject.code, false, false, false);
	...
	public void updateTest() throws Exception {
		...
		testData.accept((refSample,data) -> {
			...
			sampleToUpdate.traceInformation.modifyUser = TUResources.USER;
			sampleToUpdate.traceInformation.modifyDate = new Date();
			Sample sample = api.get().update(sampleToUpdate, TUResources.USER);
	...
	public void test2Samples() throws Exception {
		F1<Project,CC1<Sample>> newSample = p -> using(TUResources.USER, () -> TestSampleFactory.sample(TUResources.USER, p.code)); 
		CC3<TestContext,Sample,Sample> s2 = TUResources.contextResource
				.nest2((context) ->	TUResources.createProject.nest2(p -> newSample.apply(p).and(newSample.apply(p)))
