...
public class RunLaneTreatmentsAPITest {
    ...
    public static final CC3<TestContext, Run, Treatment> illuminaData = TUResources.createIlluminaRunAndReadSetRWC
            .cc3((context, exp, refRun, run, refReadSet, readSet) -> {
                ...
                Treatment treatment = context.apis().laneTreatment().save(run, t, 1, TUResources.USER);
    ...
    public void deleteTest() throws Exception {
        illuminaData.accept((__, run, treatment) -> {
            api.get().delete(run, 1, treatment.code, TUResources.USER);
    ...
    public void saveTest() throws Exception {
        TUResources.createIlluminaRunAndReadSetRWC.accept((__, exp, refRun, run, refReadSet, readSet) -> {
            Treatment t = TestRunFactory.savTreatmentForLane();
            Treatment treatment = api.get().save(run, t, 1, TUResources.USER);
    ...
    public void updateTest() throws Exception {
        illuminaData.accept((__, run, treatment) -> {
            ...
            Treatment t = api.get().update(run, 1, input, TUResources.USER);
