...
import fr.cea.ig.ngl.test.resource.RProcess;
...
public class ProcessesAPITest {
	...
	private static final CC3<Container, Process, List<Process>> data = 
//			RProcess.createProcessQCRWC
//			RProcess.createProcessQC(RApplication.contextResource)
			RApplication.contextResource
			.nest3(RProcess::createProcessQC)
	...
	public void updateStateTest() throws Exception {
		...
		data.accept((container, refProcess, processes) -> {
			State state       = new State(ProcessStateNames.IP, RConstant.USER);
			final String code = processes.iterator().next().code;
			Process proc = api.get().updateState(code, state, RConstant.USER);
