...
public class TestExperimentFactory {
    ...
    public static Experiment experimentQC(String user, Container container) {
        return experimentQC(user, Arrays.asList(container));
    }	
    ...
    public static Experiment experimentIlluminaDepot(String user, List<Container> containers) {
        Experiment exp = experimentTransformation(user,
                                                  ...
                                                  containers,
                                                  TestExperimentFactory::oneToVoidATM,
                                                  instrumentHiSeq());
        exp.atomicTransfertMethods = Iterables.zip(Iterables.range(1), containers)
        		                              .zipMap((i,c) -> oneToVoidATM(i, c))
        		                              .toList();
        exp.status               = new Valuation(user, true);
        exp.instrumentProperties = depotIlluminaInstrumentProperties();
        exp.experimentProperties = depotIlluminaExperimentProperties();
        exp.projectCodes         = Iterables.flatMap(containers, c -> c.projectCodes).toSet();
        exp.sampleCodes          = Iterables.flatMap(containers, c -> c.sampleCodes).toSet();
        return exp;
    }
    
    /**
     * Experiment is created with a project code that is the project code of the first project 
     * code of the first container and with type="nanopore-depot" and protocol="genomic-dna-ligation-sqk-lsk109-promethion-1d".
     * @param user       user
    ...
    // AJ: maybe need some improvements (generalization)
...
//        }
        exp.atomicTransfertMethods = Iterables.range(1,8).map(i -> manyToOneATM("lane", fcCode).apply(i, container)).toList();
    ...
    public static Experiment experimentQC(String user, 
                                          ...
                                          String projectCode, 
                                          List<Container> containers) {
        return experiment(user, typeCode, "qualitycontrol", protocolCode, projectCode, containers, TestExperimentFactory::oneToVoidATM, instrumentQuBit());
    }
    ...
    private static Experiment experiment(String user, 
        ...
        exp.state                  = new State(ExperimentStateNames.N, user);
        exp.sampleCodes            = Iterables.flatMap(containers, c -> c.sampleCodes).toSet();
        exp.inputContainerCodes    = Iterables.map    (containers, c -> c.code)       .toSet();
        exp.atomicTransfertMethods = Iterables.zip(Iterables.range(1), containers)
        		                              .zipMap((i,c) -> atm.apply(i, c))
        		                              .toList();
//        Iterables.zip(Iterables.range(0), containers).unzipEach((index, c) -> {
...
//        };
//    }
    private static AtomicTransfertMethod oneToVoidATM(int index, Container container) {
    	return oneToVoidContainer(index, container);
    }
...
//        state.user = c.state.user;
        State state = new State("IW-E", c.state.user);
        int tsIndex = 0;
...
//        c.state = state;
    ...
    private static OutputContainerUsed outputContainerUsed(Container c, String categoryCode) {
        ...
        ocu.locationOnContainerSupport = TestContainerFactory.supportLocation(DevAppTesting.newCode(), TestContainerFactory.TUBE);
        return ocu;
    }
...
//        String code = DevAppTesting.newCode() + "_A";
        String code = c.getCode() + "_A";
        ocu.code = code;
        ocu.categoryCode = "lane";
        ocu.locationOnContainerSupport = TestContainerFactory.supportLocation(code, "flowcell-1");
        return ocu;
    }
    ...
    private static OutputContainerUsed illuminaFlowcellOCU(Integer index, Container c, String fcCode) {
        ...
        ocu.categoryCode = "lane";
        ocu.locationOnContainerSupport = TestContainerFactory.supportLocation(fcCode, "flowcell-8", new String(""+index.intValue()), "1");
...
//    }
    ...
    // AJ: remove it and use dedicated factory
