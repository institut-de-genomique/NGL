 ...
 angular.module('home').controller('CreationKitsCtrl', ['$scope', 'datatable','mainService','tabService','$q','$http','$filter','lists','$routeParams','$location', function ($scope, datatable,mainService,tabService,$q,$http,$filter,lists,$routeParams,$location) {
	...
	function startDeleteInProgress(){
		deleteInProgress = true;
		$scope.messages = {};
	}

	function endDeleteInProgress(){
		deleteInProgress = false;
	}

	 $scope.isDeleteInProgress = function(){
		 ...
		 $q.all(promises).then(function (res) {
				...
				$q.all(promises).then(function (res) {
					...
					function setErrorDetails(){
						var details = reason.data;
						var messageDetails = isDetailsAnHtmlString() ? genericDetails() : details;
						setMessageDetails();

						//---

						function isDetailsAnHtmlString(){
							return typeof details === 'string';
						}

						function genericDetails(){
							var errorMsg = Messages('reagents.msg.boxes.unknown.error');
							return {boxes: [errorMsg]};
						}

						function setMessageDetails(){
							$scope.message.details = messageDetails;
							$scope.message.isDetails = true;
						}
					}
				 });
