angular.module('commonsServices').directive('btInput', [ '$parse', '$filter', function($parse, $filter) {
	return {
		...
		template : '<div>'
			...
			+'<textarea ng-class="inputClass" ng-model="textareaValue" ng-keydown="intercept($event)" ng-keyup="setTextareaNgModel()" rows="5" ng-paste="setTextareaNgModelFromPaste($event)" ></textarea>' 
			+ '</li>' 
			+ '</ul>' 
			+ '</div>'
			//select mode
		...
		link : function(scope, element, attr, ctrl) {
			...
			scope.setTextareaNgModelFromPaste = function($event){
				//console.log('PASTE setTextareaNgModelFromPaste......'+ $event.originalEvent.clipboardData.getData('text/plain') );
				...
				//console.log('MOUSE PASTE values='+ values);
				scope.textareaNgModel = values;
			}
			
			scope.setTextareaNgModel = function(){
				//console.log('KEYBOARD setTextareaNgModel......'+scope.textareaValue );
				var values = scope.textareaValue.split(/\s*[,;\n\t]\s*/);
				//console.log('KEYBOARD values='+ values);
				scope.textareaNgModel = values;
			}
