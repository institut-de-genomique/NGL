...
import fr.cea.ig.ngl.dao.api.StatefulAPI;
...
public class AKindOfMagic {
	...
	public <T extends DBObject, DAO extends GenericMongoDAO<T>, API extends GenericAPI<DAO, T>> T update(T t, String user) throws DAOException, APIException {
		API api = getAPI(t);
		logger.debug("using API {} to create ({},{}) {}", api, t.getCode(), t.get_id(), t);
		T r = api.update(t,user);
		logger.debug("created {} {}", r.getCode(), r.get_id());
		return r;
	}

	// Only objects with states support the updateState in an API extension.
	public <T extends DBObject, DAO extends GenericMongoDAO<T>, API extends GenericAPI<DAO, T>> T updateState(T t, State state, String user) throws DAOException, APIException {
		API api = getAPI(t);
		if (!(api instanceof StatefulAPI))
			throw new RuntimeException("" + api + " (" + api.getClass() + ") does not implement " + StatefulAPI.class);
		@SuppressWarnings("unchecked")
		StatefulAPI<T> sapi = (StatefulAPI<T>)api;
		return sapi.updateState(t.getCode(), state, user);
	}
