...
public class PropertyDefinitionDAO extends AbstractDAOMapping<PropertyDefinition> {
	...
	public List<PropertyDefinition> findUnique(Level.CODE levelCode){
		...
		PropertyDefinitionMappingQuery propertyDefinitionMappingQuery=new PropertyDefinitionMappingQuery(dataSource, sql, Version.LIGHT, new SqlParameter("l.code",Types.VARCHAR));
	...
	@SuppressWarnings("unchecked")
	public List<PropertyDefinition> findAllByObjectType(String objectTypeCode, String code, List<String> codes, String typeName, List<String> typeNames){
		String sql = "SELECT "
				+ "c.id,"
				+ "c.code,"
				+ "c.name,"
		...
		if(code != null) {
			...
			sql += " AND t.code in " + sqlInListWorkaround(codes, sqlParameters, parameters);
		}
		if(typeName != null) {
			sql += " AND c.name = ?";
