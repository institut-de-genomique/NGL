		...
		for (Content sampleUsed : container.contents) {
			try {
				if (null == tag && sampleUsed.sampleCode.equals(readSet.sampleCode)){
					...
					Index index=MongoDBDAO.findOne(InstanceConstants.PARAMETER_COLL_NAME, Index.class, DBQuery.in("typeCode", "index-illumina-sequencing","index-nanopore-sequencing").is("shortName", tag));
					if(index!=null){
