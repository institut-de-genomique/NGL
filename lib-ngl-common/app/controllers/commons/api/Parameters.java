...
public class Parameters extends DocumentController<Parameter> {
	...
	private static Query getQuery(ParametersSearchForm form) {
		...
		// !! attention....supplierName est un object => 'regex' fonctionne pas !!
		if(StringUtils.isNotBlank(form.supplierNameRegex)){
			queries.add(DBQuery.where("function() { for (var field in this.supplierName) { if (field.match(/"+ Pattern.compile(form.supplierNameRegex)+"/)) {return true};}}"));
		}
