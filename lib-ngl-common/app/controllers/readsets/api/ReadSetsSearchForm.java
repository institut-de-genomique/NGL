...
public class ReadSetsSearchForm extends DBObjectListForm<ReadSet> {
    ...
    public Query getQuery() {
        ...
        if (StringUtils.isNotBlank(this.bioinformaticValidCode)) { //all
            queries.add(DBQuery.is("biointhisaticValuation.valid", TBoolean.valueOf(this.bioinformaticValidCode)));
        }
        ...
        if(null != this.fromEvalDate && null != this.toEvalDate){
            queries.add(DBQuery.or(
                    ...
                    DBQuery.and(DBQuery.greaterThanEquals("biointhisaticValuation.date", this.fromEvalDate),DBQuery.lessThan("biointhisaticValuation.date", (DateUtils.addDays(this.toEvalDate,1))))
            ));
        }else if(null != this.fromEvalDate && null == this.toEvalDate){
            queries.add(DBQuery.or(DBQuery.greaterThanEquals("productionValuation.date", this.fromEvalDate),DBQuery.greaterThanEquals("biointhisaticValuation.date", this.fromEvalDate)));
        }else if(null != this.toEvalDate && null == this.fromEvalDate){
            queries.add(DBQuery.or(DBQuery.lessThan("productionValuation.date", (DateUtils.addDays(this.toEvalDate,1))),DBQuery.lessThan("biointhisaticValuation.date", (DateUtils.addDays(this.toEvalDate,1)))));
        }
        ...
        if (CollectionUtils.isNotEmpty(this.bioinformaticResolutionCodes)) { //all
            queries.add(DBQuery.in("biointhisaticValuation.resolutionCodes", this.bioinformaticResolutionCodes));
        }
