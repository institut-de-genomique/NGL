...
public class MembersProjectsDAO {
	...
	@Inject
	public MembersProjectsDAO(ActiveDirectoryServices services) {
	...
	public Members saveObject(Members members) throws DAOException {
		try {
			...
			services.addListUsersToGroup(MembersProjectsUtils.convertListUserMembers(members.admins), groupAdminProjectName);
			//Add admin to group
			//groupName memberOf=groupAdminName
			//groupAdminName members=groupName 
			// EJACOBY: a valider
			//services.addUsersGroupToGroup(groupProjectName, groupAdminProjectName);
	...
	public Members getObject(String code) throws DAOException {
		try {
			...
			members.admins=MembersProjectsUtils.convertListMapValue(services.getMembers(members.adminGroupName, attributeNames));
			return members;
		} catch (NamingException e) {
			throw new DAOException(e);
		}
