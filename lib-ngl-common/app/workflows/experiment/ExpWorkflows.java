...
//				expWorkflowsHelper.updateRemoveContainersFromExperiment(exp, validation, newState(contWorkflows.getContainerStateFromExperimentCategory(exp.categoryCode), validation.getUser())); 
//				expWorkflowsHelper.updateAddContainersToExperiment(exp, validation, newState("IW-E", validation.getUser()));				
//				expWorkflowsHelper.updateRemoveContainersFromExperiment(exp, validation, new State(contWorkflows.getContainerStateFromExperimentCategory(exp.categoryCode), validation.getUser())); 
...
//		} else if ("F".equals(exp.state.code)) {
			...
			if (PermissionHelper.checkPermission(validation.getUser(), "admin") && validation.getObject("updateContentProperties") != null){
				...
				validation.putObject(OBJECT_IN_DB, dbExp); // Is this really used as a return value ?
			}
		}
...
//	@Override
	...
	public void applySuccessPostStateRules(ContextValidation validation, Experiment exp, boolean updateContainerSupports) {
		...
		if (N.equals(exp.state.code)) {
//			expWorkflowsHelper.updateStateOfInputContainers(exp, newState("IW-E", validation.getUser()), validation);
...
//			expWorkflowsHelper.updateStateOfInputContainers(exp, newState("IU", validation.getUser()), validation);
			expWorkflowsHelper.updateStateOfInputContainers(validation, exp, new State("IU", validation.getUser()), updateContainerSupports);
			expWorkflowsHelper.updateStateOfInputContainerSupports(exp, validation);
//			expWorkflowsHelper.updateStateOfProcesses(exp,  newState("IP", validation.getUser()), validation);			
			expWorkflowsHelper.updateStateOfProcesses(exp,  new State("IP", validation.getUser()), validation);			
		} else if (F.equals(exp.state.code)) {
//			expWorkflowsHelper.updateStateOfInputContainers(exp, newState("IW-D", validation.getUser()), validation);
...
//			expWorkflowsHelper.rollbackOnContainers(contextValidation, newState(contWorkflows.getContainerStateFromExperimentCategory(exp.categoryCode), contextValidation.getUser()), exp.code, containerCodes);
//			expWorkflowsHelper.rollbackOnContainers(contextValidation, new State(contWorkflows.getContainerStateFromExperimentCategory(exp.categoryCode), contextValidation.getUser()), exp.code, containerCodes);
